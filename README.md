...
A GenAI-powered resume screening tool that reads PDF resumes, extracts
skills and experience, matches candidates against a job description using
NLP, ranks them with a clear explainable score, and uses **Gemini
(Google AI)** to generate a fresh recruiter summary and tailored interview
questions for every candidate — all through a simple Streamlit dashboard.

![Python](https://img.shields.io/badge/Python-3.11%2B-blue)
![Streamlit](https://img.shields.io/badge/Streamlit-Dashboard-red)
![spaCy](https://img.shields.io/badge/spaCy-NLP-09a3d5)
![Gemini](https://img.shields.io/badge/Gemini-GenAI-4285F4)
...
- 🧠 **AI-Enhanced Skill Detection** — Gemini catches skills not on the built-in list, so you're never limited to a fixed keyword set
...
- ✨ **GenAI Recruiter Summary** — Gemini writes a fresh 2–3 sentence summary of each candidate's fit
- 🎤 **GenAI Interview Questions** — Gemini generates tailored interview questions per candidate, based on their matched and missing skills
...
| GenAI | [Gemini API (Google)](https://ai.google.dev/) | Generates recruiter summaries & interview questions |
...
├── genai_generator.py         # GenAI summaries & interview questions (Gemini API)
├── requirements.txt
├── .env.example
...
### 5. Set your Gemini API key (for the AI summary/interview-question features)

The app reads a `.env` file automatically on startup (via `python-dotenv`):
...
No `export`/`setx` command needed — just save the `.env` file and run the
app. Parsing, matching, and ranking all work without this key — only the
**Generate AI Summary & Interview Questions** button needs it.
...
3. Leave **AI-enhanced skill detection** on if you have a Gemini key set
4. Upload one or more resume PDFs
5. Click **🔍 Screen Candidates**
6. View the ranked results — click any candidate to see their matched and
   missing skills, experience, and full score breakdown
7. Click **Generate AI Summary & Interview Questions** to have Gemini
   write a fresh recruiter summary and tailored interview questions for
   that candidate
...
- **Add more skills** — edit `DEFAULT_SKILLS_DB` in `extractor.py` to
  cover other domains (marketing, finance, design, etc.). Usually
  unnecessary though — AI-enhanced skill detection already catches
  skills not on this list when a Gemini key is set.
